...
namespace PoEHUD.HUD
{
    public class CustomColorDialog
    {
        ...
        public CustomColorDialog(Color color)
        {
            ...
            {
                pressed = false;
                if (int.TryParse(alphaText.Text, out int value) && alphaSlider.Value != value)
                {
                    if (value < 0)
                    {
                        value = 0;
        ...
        public bool Show()
        {
            bool result = WindowsAPI.ChooseColor(ref chooseColor);
            if (!result)
            {
                return false;
            }
        ...
        {
            const int initDialog = 0x0110;
            const int resizeDialog = 0x0005;
            ...
            Rectangle windowRect = WindowsAPI.GetClientRectangle(hWnd);
            Rectangle panelRect = alphaPanel.ClientRectangle;
            switch (msg)
            {
                case initDialog:
                    WindowsAPI.SetForegroundWindow(hWnd);
                    WindowsAPI.SetParent(alphaPanel.Handle, hWnd);
                    Rectangle screenSize = Screen.PrimaryScreen.Bounds;
                    int width = windowRect.Width + panelRect.Width + 25;
                    int height = windowRect.Height + 38;
                    WindowsAPI.MoveWindow(hWnd, screenSize.Width / 2 - width / 2, screenSize.Height / 2 - height / 2, width, height, true);
                    break;
                case resizeDialog:
                    WindowsAPI.MoveWindow(alphaPanel.Handle, windowRect.Right - windowRect.Left - panelRect.Width - 10, 0, panelRect.Width, panelRect.Height, true);
                    break;
            }

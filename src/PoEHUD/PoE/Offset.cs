...
namespace PoEHUD.PoE
{
    public class Offset
    {
        public static Offset Regular = new Offset { IgsOffset = 0,    IgsDelta = 0, ExecutableName = "PathOfExile_x64" };
        public static Offset Steam   = new Offset { IgsOffset = 0x28, IgsDelta = 0, ExecutableName = "PathOfExile_x64Steam" };
        ...
        */
        ...
        public void DoPatternScans(Memory memory)
        {
            long[] array = memory.FindPatterns(BasePtrPattern, FileRootPattern, AreaChangePattern);
            Base = memory.ReadInt(memory.AddressOfProcess + array[0] + 0x12) + array[0] + 0x16;
            System.Console.WriteLine("Base Address: " + (Base + memory.AddressOfProcess).ToString("x8"));

            FileRoot = memory.ReadInt(memory.AddressOfProcess + array[1] + 0x3) + array[1] + 0x7;
            System.Console.WriteLine("FileRoot Pointer: " + (FileRoot + memory.AddressOfProcess).ToString("x8"));

            AreaChangeCount = memory.ReadInt(memory.AddressOfProcess + array[2] + 0x25) + array[2] + 0x29;
            System.Console.WriteLine("AreaChangeCount Pointer: " + (AreaChangeCount + memory.AddressOfProcess).ToString("x8"));
        }
    }
}
